...
#pragma rs java_package_name(indrora.neural)
...
static void setColor(int c) {
	color_t color_this = colors[c];
    if (gMode == 1) {
        // sholes red
        rsgProgramFragmentConstantColor(gPFTexture, 0.9f, 0.1f, 0.1f, 0.8f);
    } else {
    	rsgProgramFragmentConstantColor(gPFTexture, color_this.red, color_this.green, color_this.blue, color_this.alpha);
    }
}
